...
export async function GET(req:Request){
  try{
    ...
    const installationRows=installations.data||[],connectionRows=(connections.data||[]).map((x:any)=>({...x,metadata:{...(x.metadata||{})}}));
    const installByBinding=new Map<string,any>();
    for(const i of installationRows){const key=String(i.license_binding_id||""),current=installByBinding.get(key);if(!current||installationScore(i)>installationScore(current))installByBinding.set(key,i)}
    ...
    const preferredBinding=bindingRows.find(hasBase);
    const preferredInstall=preferredBinding?installationRows.find((x:any)=>x.license_binding_id===preferredBinding.id):null;
    const vercel=connectionRows.find((x:any)=>x.provider==="vercel"&&x.status==="connected");
    const finalConnections=preferredInstall?.vercel_project_id?connectionRows.map((x:any)=>x.provider==="vercel"?{...x,team_id:preferredInstall.vercel_team_id||x.team_id,metadata:{...(x.metadata||{}),team_id:preferredInstall.vercel_team_id||x.metadata?.team_id||null,teams:[],team_locked:true}}:x):connectionRows;
    const ids=installationRows.map((x:any)=>x.id);let events:any[]=[],releases:any[]=[];
    ...
    const latest=preferredInstall?.release_version?(latestUpdate||latestBase):latestBase;
    const slim=(x:any)=>x?{version:x.version,releaseId:x.releaseId,publishedAt:x.publishedAt,sourceCommit:x.sourceCommit,title:x.title,description:x.description,changelog:x.changelog,customerNotes:x.customerNotes,severity:x.severity,required:x.required,rollout:x.rollout,channel:x.channel}:null;
    return Response.json({settings,bindings:bindingRows,connections:finalConnections,installations:installationRows,events,releases,latestRelease:slim(latest),latestBase:slim(latestBase),latestUpdate:slim(latestUpdate)},{headers:{"cache-control":"no-store"}});
