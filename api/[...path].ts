...
const requestWithMasterAuthority = (input: Record<string, unknown>) => new Request(`${siteUrl}/api/license/issue`, {
  method: "POST",
  headers: { "content-type": "application/json", authorization: `Bearer ${process.env.MASTER_API_TOKEN || ""}` },
...
const apiIndex = (res: ServerResponse) => sendJson(res, 200, {
  ...
  endpoints: {
    health: `${apiBase}/license/v1/health`, revision: `${apiBase}/license/v1/revision`, publicKey: `${apiBase}/license/v1/public-key`,
    register: `${apiBase}/license/v1/register`, activate: `${apiBase}/license/v1/activate`, validate: `${apiBase}/license/v1/validate`, issue: `${apiBase}/license/issue`,
  },
});
const licenseRoute = async (req: IncomingMessage, res: ServerResponse, pathname: string) => {
  ...
  if (pathname.endsWith("/validate") && req.method === "POST") return sendJson(res, 200, await validateLicense(await bodyOf(requestFromNode(req))));
  if ((pathname.endsWith("/activate") || pathname.endsWith("/register")) && req.method === "POST") return sendJson(res, 200, await validateLicense({ ...(await bodyOf(requestFromNode(req))), activate: true }));
  if (pathname.endsWith("/issue") && req.method === "POST") {
    const request = requestFromNode(req); const input = await bodyOf(request);
    try { return sendJson(res, 200, await issueLicense(request, input)); }
    catch (error) { if (!(error instanceof AuthorityError) || error.status !== 401) throw error; await requireAdmin(request); return sendJson(res, 200, await issueLicense(requestWithMasterAuthority(input), input)); }
  }
  return false;
};
export default async function api(req: IncomingMessage, res: ServerResponse) {
  const requestId = String(req.headers["x-request-id"] || randomUUID()); res.setHeader("x-request-id", requestId);
  try {
    const rawPath = String(req.url || "/").split("?")[0];
    const pathname = rawPath === "/api" || rawPath === "/api/" ? "/api" : rawPath.startsWith("/api/") ? rawPath : `/api${rawPath.startsWith("/") ? rawPath : `/${rawPath}`}`;
    if (pathname === "/api/admin" || pathname === "/api/admin/") return sendText(res, 200, readFileSync(new URL("../web/admin.html", import.meta.url), "utf8"), "text/html; charset=utf-8");
    ...
    if (adminControlMatch && req.method === "POST") return sendJson(res, 200, await adminLicenseControl(requestFromNode(req), decodeURIComponent(adminControlMatch[1]), String((await bodyOf(requestFromNode(req))).action || "")));
    if (pathname.startsWith("/api/license/")) { const handled = await licenseRoute(req, res, pathname); if (handled !== false) return handled; }
    ...
    if (pathname === "/api/admin-products-ui" || pathname === "/api/admin-control-ui" || pathname === "/api/admin-settings" || pathname === "/api/admin-extended" || pathname === "/api/admin-extended/") return sendJson(res, 404, { error: "Legacy API endpoint removed" });
    ...
    const originalUrl = req.url; req.url = pathname + (String(originalUrl || "").includes("?") ? String(originalUrl).slice(String(originalUrl).indexOf("?")) : "");
    try { const { handler } = await import("../src/server.js"); return await handler(req, res); }
    finally { req.url = originalUrl; }
  } catch (error) {
    const status = error instanceof AuthorityError ? error.status : 500;
    sendJson(res, status, { error: status >= 500 ? "Master service error" : error instanceof Error ? error.message : "Request failed", ...(error instanceof AuthorityError && error.code ? { code: error.code } : {}), requestId });
  }
}