...
async function updateSettings(req: IncomingMessage) {
  await ensureSettingsTable();
...
async function releaseSource(kind: string) {
  ...
  const repo = base ? "lucaskerim123/V1-vercel-base" : "lucaskerim123/V1-vercel-engine";
  const branch = base ? "base-release" : "release-updates";
  ...
  if (!r.ok) throw new Error(String((data as { message?: unknown })?.message || "GitHub source lookup failed"));
  const c = Array.isArray(data) ? data[0] as Record<string, unknown> | undefined : undefined;
  if (!c?.sha) throw new Error(`No commits found for ${repo} / ${branch}`);
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    if (action === "settings") {
      const s = req.method === "GET" ? await getSettings() : await updateSettings(req);
      return json(res, 200, { ok: true, settings: s, database: true, settings_found: true });
    }
    if (action === "health") {
      const s = await getSettings();
      return json(res, 200, { ok: true, database: true, settings_found: true, settings: s, api: "License Master V2", services: { billing: Boolean(BILLING), deployer: Boolean(DEPLOYER) }, endpoints: { base: "/api", versioned: "/api/v1", billing: { products: "/api/v1/products", issue: "/api/v1/license/issue", validate: "/api/v1/license/validate", releases: "/api/v1/releases" }, deployer: { releases: "/api/v1/releases", installations: "/api/v1/installations", deployments: "/api/v1/deployments", execute: "/api/v1/deployments/execute", sync: "/api/v1/deployments/sync" }, updater: { releases: "/api/v1/releases", revision: "/api/v1/license/revision" } } });
    }
