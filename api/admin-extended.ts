...
const RELEASE_UPDATE_BRANCH = "release-updates";
...
async function releaseSource(kind: string) {
  ...
  const repo = base ? RELEASE_BASE_REPOSITORY : RELEASE_UPDATE_REPOSITORY;
  const branch = base ? RELEASE_BASE_BRANCH : RELEASE_UPDATE_BRANCH;
