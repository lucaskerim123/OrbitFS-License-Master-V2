...
type Settings = { id:string; issuer:string; audience:string; entitlement_ttl_seconds:number; grace_seconds:number; api_mode:string; allow_offline_grace:boolean; revision:number; updated_at:string };
...
async function getSettings():Promise<Settings>{
  ...
  const row=(await db!.query<Settings>("select id,issuer,audience,entitlement_ttl_seconds,grace_seconds,api_mode,allow_offline_grace,revision,updated_at from master_license_settings where id='primary'")).rows[0];
...
async function updateSettings(input:Record<string,unknown>):Promise<Settings>{
  ...
  const row=(await db!.query<Settings>(`update master_license_settings set issuer=$1,audience=$2,entitlement_ttl_seconds=$3,grace_seconds=$4,api_mode=$5,allow_offline_grace=$6,revision=$7,updated_at=now() where id='primary' returning id,issuer,audience,entitlement_ttl_seconds,grace_seconds,api_mode,allow_offline_grace,revision,updated_at`,[issuer,audience,Math.max(60,Math.floor(ttl)),Math.max(0,Math.floor(grace)),mode,allowOfflineGrace,revision])).rows[0];
