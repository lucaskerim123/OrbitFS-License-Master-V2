...
export default async function billing(req: IncomingMessage, res: ServerResponse) {
  ...
  // Billing is a separate application and a separate Supabase/Auth authority.
  // This endpoint authenticates only the Billing service. Master administrator
  // identity is handled exclusively by the Master Supabase project through
  // /api/auth/login and /api/admin/me.
  return json(res, 200, {
    ok: true,
