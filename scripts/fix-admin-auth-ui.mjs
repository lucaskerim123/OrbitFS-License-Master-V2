...
// The Vercel build currently uses src/server.ts as the canonical API entrypoint.
// Put the admin read/release routes directly into that handler so they cannot be swallowed by the catch-all function routing.
const serverPath = new URL('../src/server.ts', import.meta.url);
let server = readFileSync(serverPath, 'utf8');
const marker = '    // CANONICAL_ADMIN_ROUTES_V2';
const adminRoutes = `${marker}
    ...
    if (path === "/api/admin/releases" && req.method === "POST") {
      ...
      const release = (await query<JsonObject>(
        `insert into releases (id,component,version,channel,status,title,description,changelog,customer_notes,internal_notes,severity,required,rollout,minimum_version,rollback_version,schema_version,checkpoint_required,components,manifest,permissions,compatibility,source_commit) values(gen_random_uuid(),$1,$2,$3,'draft',$4,'',$5,'','',$6,false,'internal',null,null,'1',false,$7,'{}','{}','{}',$8) returning *`,
        [component, version, channel, String(input.title || `OrbitFS ${version}`), String(input.changelog || ""), String(input.severity || "normal"), input.components || [], input.sourceCommit || null]
      )).rows[0];
