...
import { adminLicenseControl, licenseSettings } from './new-api-admin.js';
...
export async function handleAdminConsole(req: Request, pathname: string): Promise<Response | null> {
  ...
  if (pathname === '/admin/products' && req.method === 'GET') return json({ products: (await query<any>('select * from license_products order by sort_order asc, code asc limit 500')).rows });
  if (pathname === '/admin/settings' && req.method === 'GET') return json(await licenseSettings(req));
  if (pathname === '/admin/settings' && req.method === 'POST') return json(await licenseSettings(req, await body(req)));
  return null;
}
