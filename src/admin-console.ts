...
const masterRequest = (input: Record<string, unknown>) => new Request('https://incendiarynetworks.cc/api/license/issue', { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${process.env.MASTER_API_TOKEN || ''}` }, body: JSON.stringify(input) });
const PRODUCT_CODES = ['orbitfs_base','orbitfs_mcp','orbitfs_apex'];
...
export async function handleAdminConsole(req: Request, pathname: string): Promise<Response | null> {
  ...
  if (pathname === '/api/admin/licenses/issue' && req.method === 'POST') { const input = await body(req); return json(await issueLicense(masterRequest(input), input), 201); }
  if (pathname === '/api/admin/releases' && req.method === 'GET') return json({ releases: (await query<any>('select * from releases order by updated_at desc limit 500')).rows });
  if (pathname === '/api/admin/releases' && req.method === 'POST') {
  ...
  const releaseAction = pathname.match(/^\/api\/admin\/releases\/([^/]+)\/(validate|publish|pause|withdraw)$/);
  ...
  if (pathname === '/api/admin/deployments' && req.method === 'GET') return json({ jobs: (await query<any>('select * from deployment_jobs order by created_at desc limit 200')).rows });
  if (pathname === '/api/admin/installations' && req.method === 'GET') return json({ installations: (await query<any>('select * from orbitfs_installations order by updated_at desc limit 500')).rows });
  if (pathname === '/api/admin/products' && req.method === 'GET') return json({ products: (await query<any>('select * from license_products where code = any($1::text[]) order by sort_order asc, code asc limit 500', [PRODUCT_CODES])).rows });
  if (pathname === '/api/admin/settings' && req.method === 'GET') return json({ settings: (await query<any>("select * from master_license_settings where id='primary' limit 1")).rows[0] || null });
  return null;
}
