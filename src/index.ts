...
function cors(request: Request) {
  ...
  return { "access-control-allow-origin": origin,
    "access-control-allow-headers": "content-type,authorization,x-orbitfs-order-ref",
    "access-control-allow-methods": "GET,POST,OPTIONS", "vary": "origin" };
}

function routeRelease(path: string) {
  const match = path.match(/^\/api\/v1\/releases\/([^/]+)(?:\/(artifact|publish|control))?$/);
...
export default { async fetch(request: Request, env: Env) {
  ...
  try {
    ...
    const release = routeRelease(url.pathname);
